...
describe("CodexAdapter", () => {
  ...
  test("transformMemory converts MEMORY.md to AGENTS.md", () => {
  ...
  test("validate memory: wrong filename fails", () => {
    ...
    const result = adapter.validate("memory", memory);
    expect(result.valid).toBe(false);
    expect(result.errors).toContain("Memory file must be named AGENTS.md for Codex");
  });
