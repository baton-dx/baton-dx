...
`,
            ...
            await expect(
                resolveProfileChain(manifestA, "./profiles/profile-a", tempDir),
...
`,
            ...
            await expect(
                resolveProfileChain(manifest, "./profiles/level-0", tempDir),
...
`,
    ...
    describe("hard error on extends resolution failure", () => {
        it("throws descriptive error when extends target does not exist", async () => {
            ...
            await expect(
                resolveProfileChain(childManifest, "./profiles/child", tempDir),
        ...
        it("still throws max-depth error for excessively deep chains", async () => {
            ...
            await expect(
                resolveProfileChain(manifest, "./profiles/deep-0", tempDir),
            ).rejects.toThrow(/exceeds maximum depth/);
        });
