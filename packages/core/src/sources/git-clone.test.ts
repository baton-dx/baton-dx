...
describe("concurrent clone serialization", () => {
    ...
    it("serializes 4+ concurrent clones to the same cache path", async () => {
        ...
        const mockClone = vi.fn().mockImplementationOnce(() => {
        ...
        const mockCheckIsRepo = vi
            .fn()
            .mockRejectedValueOnce(new Error("not a git repo")) // call 1, line 158: no cache
            .mockRejectedValueOnce(new Error("not a git repo")) // call 1, line 222: still no cache
            .mockResolvedValue(true); // calls 2-4: cache populated by call 1
        ...
        // Start 4 concurrent calls — same URL (simulates 4 profiles from same repo)
        const call1 = cloneGitSource({
            url: "https://example.com/multi-concurrent.git",
            useCache: true,
        });
        const call2 = cloneGitSource({
            url: "https://example.com/multi-concurrent.git",
            useCache: true,
            subpath: "profiles/react",
        });
        const call3 = cloneGitSource({
            url: "https://example.com/multi-concurrent.git",
            useCache: true,
            subpath: "profiles/nextjs",
        });
        const call4 = cloneGitSource({
            url: "https://example.com/multi-concurrent.git",
            useCache: true,
            subpath: "profiles/base",
        });
        ...
        // All 4 should succeed and share the same cache path
        for (const result of results) {
            expect(result.cachePath).toBe(results[0].cachePath);
        }
        // Only one git.clone call — calls 2-4 reused cache via fetch+reset
