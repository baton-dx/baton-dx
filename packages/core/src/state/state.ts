...
export const placementStateSchema = z.object({
    ...
    placed_files: z
        .object({
            ...
            ides: z
                .array(z.string())
                .describe("Paths placed for IDE platforms (.vscode/, .idea/, ...)"),
            files: z
                .array(z.string())
                .describe(
                    "Paths placed from profile files sections (biome.json, tsconfig.json, ...)",
                ),
            includes: z
                .array(z.string())
                .optional()
                .describe("Paths placed in .baton/includes/ for link/reference mode includes"),
        })
    ...
    mcp_servers: z
        ...
        .optional()
        .describe(
            "Baton-managed MCP server names per tool key (e.g., { 'zed': ['github', 'fs'] })",
        ),
});
