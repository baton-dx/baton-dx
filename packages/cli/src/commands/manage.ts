        ...
        for (const [weight, group] of weightGroups) {
            if (group.length > 1) {
                ...
                const nonRelated = group.filter((m) => {
                    return !group.some(
                        (other) =>
                            other !== m && (m.extends === other.name || other.extends === m.name),
                    );
                });
                if (nonRelated.length > 1) {
                    const names = nonRelated.map((m) => m.name).join(", ");
                    conflicts.push(
                        `  ⚠ weight=${weight}: ${names} — same merge weight, last-wins applies`,
                    );
                }
            }
        }
