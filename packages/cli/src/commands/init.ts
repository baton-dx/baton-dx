    ...
    async run({ args }) {
        ...
        if (args.profile) {
            ...
            if (globalSources.length === 0) {
                ...
                p.note(
                    `Using default source: ${defaultSource.name}\n${defaultSource.url}`,
                    "Source",
                );
        ...
        let gitignoreCategories: GitignoreCategories = {
            "ai-tools": true,
            ides: true,
            files: false,
        };
        ...
        if (gitignoreCategories.ides)
            sections.push({ label: "ides", patterns: collectIdePatterns() });
...
async function showProfileIntersections(profileSources: string[]): Promise<void> {
    ...
    for (const sourceString of profileSources) {
        try {
            ...
            const profileManifest = await loadProfileManifest(profileManifestPath).catch(
                () => null,
            );
